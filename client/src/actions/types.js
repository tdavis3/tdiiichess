export const SET_ALERT = "SET_ALERT";
export const REMOVE_ALERT = "REMOVE_ALERT";

export const REGISTER_SUCCESS = "REGISTER_SUCCESS";
export const REGISTER_FAIL = "REGISTER_FAIL";
export const USER_LOADED = "USER_LOADED";
export const CLEAR_USER = "CLEAR_USER";
export const AUTH_ERROR = "AUTH_ERROR";
export const LOGIN_SUCCESS = "LOGIN_SUCCESS";
export const LOGIN_FAIL = "LOGIN_FAIL";
export const LOGOUT = "LOGOUT";
export const CHANGE_EMAIL = "CHANGE_EMAIL";

export const GET_USER_ANALYTICS = "GET_USER_ANALYTICS";
export const GET_ADMIN_ANALYTICS = "GET_ADMIN_ANALYTICS";


export const SET_TOURNAMENTS_LOADING = "SET_TOURNAMENTS_LOADING";
// Tournament actions
export const GET_TOURNAMENTS = "GET_TOURNAMENTS";
export const CREATE_TOURNAMENT = "CREATE_TOURNAMENT";
export const EDIT_TOURNAMENT = "EDIT_TOURNAMENT";
export const DUPLICATE_TOURNAMENT = "DUPLICATE_TOURNAMENT";
export const DELETE_TOURNAMENT = "DELETE_TOURNAMENT";
export const CLEAR_TOURNAMENTS = "CLEAR_TOURNAMENTS";
// Success or Error
export const TOURNAMENTS_SUCCESS = "TOURNAMENTS_SUCCESS";
export const TOURNAMENTS_ERROR = "TOURNAMENTS_ERROR";


export const SET_SECTIONS_LOADING = "SET_SECTIONS_LOADING";
export const STOP_SECTIONS_LOADING = "STOP_SECTIONS_LOADING";
// Section actions
export const GET_SECTIONS = "GET_SECTIONS";
export const CREATE_SECTION = "CREATE_SECTION";
export const EDIT_SECTION = "EDIT_SECTION";
export const DUPLICATE_SECTION = "DUPLICATE_SECTION";
export const MOVE_SECTION = "MOVE_SECTION";
export const DELETE_SECTION = "DELETE_SECTION";
export const CLEAR_SECTIONS = "CLEAR_SECTIONS";
// Success or Error
export const SECTIONS_SUCCESS = "SECTIONS_SUCCESS";
export const SECTIONS_ERROR = "SECTIONS_ERROR";


export const SET_PLAYERS_LOADING = "SET_PLAYERS_LOADING";
export const STOP_PLAYERS_LOADING = "STOP_PLAYERS_LOADING";
// Player actions
export const GET_PLAYERS = "GET_PLAYERS";
export const CREATE_PLAYER = "CREATE_PLAYER";
export const EDIT_PLAYER = "EDIT_PLAYER";
export const MOVE_PLAYER = "MOVE_PLAYER";
export const DELETE_PLAYER = "DELETE_PLAYER";
export const CLEAR_PLAYERS = "CLEAR_PLAYERS";
// Success or Error
export const PLAYERS_SUCCESS = "PLAYERS_SUCCESS";
export const PLAYERS_ERROR = "PLAYER_ERROR";


export const SET_USCF_SCRAPER_LOADING = "SET_USCF_SCRAPER_LOADING";
export const USCF_SCRAPER_SUCCESS = "USCF_SCRAPER_SUCCESS";
export const CLEAR_SCRAPER = "CLEAR_SCRAPER";

export const GENERATE_PAIRINGS = "GENERATE_PAIRINGS";
export const GET_PAIRINGS = "GET_PAIRINGS";
export const DELETE_PAIRING = "DELETE_PAIRING";
export const PAIRING_ERROR = "PAIRING_ERROR";
